...
- **核心邏輯：**
    1. **資料獲取：** 程式啟動時，讀取 MoneyDJ 網站上的所有台股股號。
    2. **排除清單：** 依據股票名稱或類別，排除所有 ETF、美債等非「個股」股號。
    3. **首次執行：**
        - 從 TDCC 網站抓取所有符合條件的股號。
        - 對每個股號，抓取該股號在網站上所能查詢到的**過去一年**所有日期的數據。
        - 下載的資料應以日期排序（最舊到最新）。
    4. **後續執行：**
        - 檢查本地資料夾，對於已存在的股號資料夾，僅抓取**新**發布的日期資料，不重新下載已存在的日期。
- **輸出：**
    - **資料夾架構：** 一個股號對應一個資料夾。
...
- **核心邏輯：**
    1. **日期比對：** 對於使用者輸入的 `起始日期` 和 `結束日期`，在本地資料庫中尋找**大於或等於**該日期且最接近的**週數**數據。
    2. **異常處理：** 如果在查詢時，大於該日期的資料還不存在，則回退到尋找**小於**該日期最接近的週數數據，並在輸出中附加 `Warning` 訊息。
    3. **數據整理：**
        - 將選定期間的數據，根據 TDCC 網站的持股分級，分別整理成三個獨立的表格：
            ...
            - **表格 3：** 占集保庫存數比例 ( % )。
    4. **圖表疊加：**
        - 從 Wearn.com 網站抓取該股號在**同一期間**的週 K 線圖與成交量數據。
        - 將週 K 線圖與成交量圖，分別與上述三張表格**疊加**在同一張圖表上。將週 K 線圖**作為背景圖片**疊加在圖表上周K的日期要和圖表日期對上(相同的Work Week即可)
- **輸出：**
    - **格式：**一個 Excel 檔案，**一個工作表中包含三個表格**。
    - **內容：** 三個資料表格與三張對應的圖表，圖表應包含週 K 線與成交量數據。
...
- **核心邏輯：**
    1. **數據分級：** 根據持股張數，將數據區分為 15 個級距，對應到三種不同的類別進行繪圖。
    2. **繪圖處理：**
        - 將三種指標（人數、股數、占比）各自繪製成獨立的**趨勢曲線圖**，總共三張圖。
        ...
        - **刻度優化：**
            ...
            - **動態刻度 (Dynamic Step Size):** 程式需判斷曲線變化。如果一段區間的曲線趨近於水平線（連續 3 個數據點**的標準差小於 Y 軸總範圍的 1%** 時），則**自動縮小 Y 軸刻度**，以放大微小的變化，使趨勢更清晰可見。
    3. **三種分類繪圖：**
        - **類別一：** 一般定義 (股數)
...
這份 PRD 旨在提供一個清晰的開發藍圖，希望能幫助您以「Vibe coding」的直覺與熱情順利完成專案！
